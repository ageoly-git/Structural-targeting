...
bet ${T1} ${MASK_ADJ_DATA_DIR}/${SUB}_bet.nii.gz -f 0.25 -R -B
...
fslmaths ${OUT_DIR}/NTV_${MASK}.nii.gz  -bin ${OUT_DIR}/NTV_${MASK}.nii.gz
gunzip ${OUT_DIR}/*nii.gz


# -----------------------------------------------End of ANTs Registration. Do not remove the following command-----------------------------------------------

fslstats -t ${OUT_DIR}/NTV_${MASK}.nii -c >  ${OUT_DIR}/NTV_${MASK}_CRD.txt
done
